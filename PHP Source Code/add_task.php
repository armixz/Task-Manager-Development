...
$task_name_err = $task_description_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($task_name_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            mysqli_stmt_bind_param($stmt, "ssss", $param_board_id, $param_tname, $param_tdescription, $param_is_completed);
            
            // Set parameters
            $param_board_id = $board_id;
            $param_tname = $task_name;
            $param_tdescription = $task_description;
            $param_is_completed = '0';

            // Attempt to execute the prepared statement
            if(mysqli_stmt_execute($stmt)){
                // Redirect to login page
                header("location: show_board.php?bid=" . $board_id . '&bna=' . $board_name);
            } else{
...
<body class="d-flex justify-content-center pt-5">
    <div class="card col-3">
        ...
        <div class="card-body d-flex justify-content-center bg-light text-dark rounded"
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?bna=" . $board_name); ?>" method="post">
                    ...
                    <div class="card-footer form-group mt-5">
                        <input type="submit" class="col-12 btn btn-primary my-1" value="Add">
                        <!-- <input type="submit" class="btn btn-secondary ml-2" value="Cancel"> -->
                        <p class="text-center mt-2"><a class="text-danger" href="show_board.php?bid= <?php echo $board_id; ?>">Cancel</a></p>
                    </div>
